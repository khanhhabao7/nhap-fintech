...
def host_state():
    ...
    # Tạo danh sách players đúng format cho frontend
    players_list = []
    for i, proj in enumerate(room['players']):
        if proj:
            players_list.append({
    ...
    return jsonify({
        ...
        'max_players': room['num_players'],
        'logs': room.get('logs', [])[-40:],
        'rankings': [],  # Giữ cho tương thích cũ
        'players': players_list,  # THÊM field này cho host.html
        'joinLinks': join_links,  # THÊM field này
        'name': room.get('name', 'Game Room'),  # THÊM field này
        'ended': room.get('game_ended', False),  # THÊM field này
        'currentPhase': room['phase'],  # THÊM field này
        'can_start_deck': room['status'] == 'waiting_for_projects' and room.get('submitted_players', 0) >= 2,
        'submitted_count': room.get('submitted_players', 0),
        'phase_details': room.get('phase_details', [])  # THÊM field này
    })
